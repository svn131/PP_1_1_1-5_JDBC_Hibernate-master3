...
//                ") ENGINE=InnoDB DEFAULT CHARSET=utf8";
        try (Statement statement = util.getConnection().createStatement()) {
            statement.executeUpdate("CREATE TABLE IF NOT EXISTS `mydbtest`.`user` (`id` bigint(19) NOT NULL AUTO_INCREMENT,`name` varchar(45) NOT NULL,`lastName` varchar(45) NOT NULL,`age` tinyint(3) NOT NULL,PRIMARY KEY (`id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8");
...
//        }
    ...
    public void saveUser(String name, String lastName, byte age) {
        String sql = "INSERT INTO `mydbtest`.`user` (`name`, `lastName`, `age`) VALUES ('"+name+"', '"+lastName+"', "+age+")";

        try (PreparedStatement preparedStatementstatement = util.getConnection().prepareStatement(sql)) {
            preparedStatementstatement.executeUpdate(sql);
    ...
    public void removeUserById(long id) {
        String sql = "DELETE FROM `mydbtest`.`user` WHERE `id` = "+id;
